...
<html>
	<head>
		<meta charset="utf-8">
		<title>Mobile Innovation | 購物車</title>
				...
				<div class="container">
					<div class="row">
						<div class="main col-md-12">
							<h1 class="page-title margin-top-clear">購物車</h1>
							<div class="space"></div>
							...
							<table class="table cart table-hover table-striped">
								...
								<tbody>
									<c:forEach var="item" items="${sessionScope.cart.items}">
									<tr>
										<td class="remove"><input type="checkbox" value="${item.key}" name="itemCheck" /></td>
										<td class="product">${item.value.product.name}</td>
										<td class="price">${item.value.product.price}</td>
										<td class="quantity">
											<div class="form-group">
												<INPUT type="text" class="form-control" value="${item.value.number}" name="number${item.key}" />
											</div>											
										</td>
										<td class="amount">${item.value.cost}</td>
									</tr>
							...
							<div class="text-right">
								<a class="btn btn-group btn-default btn-sm" onclick="modifyNumber();">更新購物車</a>
								<a class="btn btn-group btn-default btn-sm" onclick="deleteCartItem();">移除</a>
								<a class="btn btn-group btn-default btn-sm" onclick="clearCart();">清除購物車</a>
								<a class="btn btn-group btn-default btn-sm" onclick="javascript:location.href='order_index?method=index'">結帳</a>
					...
					<div class="row">
						...
						<div class="col-md-6">
							<blockquote class="inline">
								<p class="margin-clear">設計不只是外觀與感覺，設計是產品如何運作。</p>	
